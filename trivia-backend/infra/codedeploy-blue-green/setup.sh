...
aws cloudformation deploy --region us-east-1 --template-file packaged-template.yaml --stack-name TriviaBackendHooksTest --tags project=nike-workshop --capabilities CAPABILITY_NAMED_IAM --tags project=nike-workshop --parameter-overrides TriviaBackendDomain=api-test.nike-workshop.com

aws cloudformation deploy --region us-east-1 --template-file packaged-template.yaml --stack-name TriviaBackendHooksProd --tags project=nike-workshop --capabilities CAPABILITY_NAMED_IAM --tags project=nike-workshop --parameter-overrides TriviaBackendDomain=api.nike-workshop.com

cd ../codedeploy-blue-green

# Provision ECS and CodeDeploy resources

aws ecs create-cluster --region us-east-1 --cluster-name default --tags key=project,value=nike-workshop
...
sed -i "s|<PLACEHOLDER>|$ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/nike-workshop-backend:latest|g" build/task-definition-test.json build/task-definition-prod.json
